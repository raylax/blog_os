...
unsafe fn active_level4_table(physical_memory_offset: VirtAddr) -> &'static mut PageTable {
    ...
    // 读取CR3寄存器的值，即根页表的物理地址
    let (level4_table_frame, _) = Cr3::read();

    // 将物理地址转换为虚拟地址
    let p_address = level4_table_frame.start_address();
    let v_address = physical_memory_offset + p_address.as_u64();
    let page_table_ptr: *mut PageTable = v_address.as_mut_ptr();
